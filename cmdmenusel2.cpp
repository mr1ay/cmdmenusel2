...
int main(int argc, char *argv[])
{
    if (argc <= 2)
    {
cout << "CmdMenuSel2 was written by mr1ay to get different output " << endl
     << "when right-clicking in CmdMenusel." << endl
     << "CmdMenuSel2(v2.1)= CmdMenuSel(V1.4)+(v0.1)" << endl << endl
     << "left click on mouse input returns errorlevel 1."<< endl 
     << "right click returns the errorlevel as 100x. "<< endl << endl
     << "For example,"<< endl
     << "if we click on the 3rd text with the left mouse, errorlevel = 3. "<< endl
     << "if we right click errorlevel=300"<< endl << endl
     << "Features from cmdmenusel:" << endl
     ...
     << "or keyboard. The menu colors can be choosen ." << endl
     << "An exit code(ErrorLevel) is returned denoting the position the selected" << endl
     << "option was specified in." << endl << endl
     << "USAGE:" << endl << endl
     << "CmdMenuSel 0F3F {\"Text 1\"} [\"Text 2\"] [....] " << endl << endl << endl
     << "All text string are trimmed to the console width. The number of specified" << endl
     << "texts must be less than or equal to the number of lines in the console." << endl << endl
     << "\"0F3F\" denotes the colors to be used, they are four hex digits" << endl
     << "(like 0 - 9 and A - F). First two digits:" << endl  
     << " +background text color and foreground text color" << endl
     << "The last two numbers are the hover colors:" << endl
     << " +background hover color and foreground hover color" << endl << endl
     << "note: this program supports the same colors for the entire menu." << endl  
     << " cmdmenusel2 does not support separate colors for each text," << endl  
     << " so you can use getinput.exe." << endl << endl
     << "See \"COLOR /?\" for color code listings." << endl << endl
     << "The default colors are used if the:" << endl
     << "  String contains characters outside of 0-9,A-F or a-f." << endl
     << "  String isn't exactly four characters." << endl
     << "  Selected or unselected background and foreground colors are the same." << endl
     << "  Selected and unselected background colors are the same." << endl << endl
     ...
     << "Select:" << endl
     << "SPACE" << endl << "ENTER" << endl << "LEFT_CLICK"<< "RIGHT_CLICK" << endl << endl
     << "NAVIGATE:" << endl
...
return 0;
    ...
    if (retcode != 0)
    {
        ...
        for (int i = 2; i < argc ; i++)
        {
            ...
            if (!FillConsoleOutputAttribute(hConsoleOutput,wStandard,
                sbiConsoleInfo.dwSize.X, coWritePos, &dwInputEventsRead))
            {
                cerr << "Could not color cells in console with FillConsoleOutputAttribute()"
                     << " Sys Error: " << GetLastError() << endl;
                retcode = 0;
                break;
            }
    ...
    while (retcode == 10000)
    {
        if (bRedraw)
        {
            if (!FillConsoleOutputAttribute(hConsoleOutput,wStandard,
                sbiConsoleInfo.dwSize.X, coWritePos, &dwInputEventsRead))
            {
                cerr << "Could not color cells in console buffer with FillConsoleOutputAttribute()"
                     << " Sys Error: " << GetLastError() << endl;
                retcode = 0;
                continue;
            }
 ...
 else if (irUserInput.Event.MouseEvent.dwButtonState & RIGHTMOST_BUTTON_PRESSED)
{
    // Sağ fare düğmesine tıklama işlemleri

    if (irUserInput.Event.MouseEvent.dwMousePosition.Y >= coMenuPos.Y
        && irUserInput.Event.MouseEvent.dwMousePosition.Y < coMenuPos.Y + wMenuItems)
    {
        // Sağ fare düğmesine tıklanması durumunda geri dönüş değerini 100 ile çarpıp ayarlayın
        retcode = (wMenuSel + 1) * 100;
    }

    // Diğer fare tıklama işlemleri
    if (irUserInput.Event.MouseEvent.dwMousePosition.Y >= coMenuPos.Y
        && irUserInput.Event.MouseEvent.dwMousePosition.Y < coMenuPos.Y + wMenuItems)
    {
        // Fare konumunu işle ve yeniden çizim gerekiyorsa bRedraw'u true yap
        wLastMousePos = wMenuSel;
        wMenuSel = irUserInput.Event.MouseEvent.dwMousePosition.Y - coMenuPos.Y;
        if (!bMouseDown)
        {
